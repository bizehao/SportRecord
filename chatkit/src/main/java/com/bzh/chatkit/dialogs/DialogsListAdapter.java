...
public class DialogsListAdapter<DIALOG extends IDialog>
    ...
    //置顶和删除的监听
    public interface OnSlidingMenuClickListener<DIALOG extends IDialog> {
        void onSetTopClick(DIALOG dialog);
    ...
    public static class DialogViewHolder<DIALOG extends IDialog> extends BaseDialogViewHolder<DIALOG> {
        ...
        //新增
        protected SlidingMenu slidingMenu; //横向滑动菜单
        protected TextView menuSetTop; //滑动菜单-置顶
        protected TextView menuDelete; //滑动菜单-删除
        ...
        public void onBind(final DIALOG dialog) {
                ...
                public boolean onTouch(View view, MotionEvent event) {
                    switch (event.getAction()) {
                        case MotionEvent.ACTION_DOWN:
                            slidingMenu.downTime = System.currentTimeMillis();
                            if (slidingMenu.getmOpenMenu() != null && slidingMenu.getmOpenMenu().isOpen() && slidingMenu.getmOpenMenu() != slidingMenu) { //处理上一个打开的
                                slidingMenu.closeOpenMenu();
                                slidingMenu.carriedOut = true;
                            }else {
                                slidingMenu.carriedOut = false;
                            }
                            break;
                        case MotionEvent.ACTION_UP:
                            int scrollX = slidingMenu.getMenuScrollX();
                            if (System.currentTimeMillis() - slidingMenu.downTime <= 200 && scrollX == 0) { //点击事件
            ...
            container.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    *//*if (slidingMenu.getmOpenMenu() != null && slidingMenu != slidingMenu.getmOpenMenu()) {
                        slidingMenu.closeOpenMenu();
                        return;
                    }*//*
                    if (slidingMenu.isOpen()) {
            ...
            //置顶的监听
            menuSetTop.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (onSlidingMenuClickListener != null) {
                        onSlidingMenuClickListener.onSetTopClick(dialog);
                        slidingMenu.closeMenu();
                    }
                }
            });
